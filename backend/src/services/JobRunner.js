...
export class JobRunner {
  ...
  async runJob(jobId) {
    ...
    // Get job details
    const jobStmt = db.prepare('SELECT * FROM jobs WHERE id = ? AND status = "active"');
    ...
    // Update job status
    const updateJobStmt = db.prepare('UPDATE jobs SET status = "running", last_run = CURRENT_TIMESTAMP WHERE id = ?');
    updateJobStmt.run(jobId);

    this.runningJobs.set(jobId, { runId, startTime: Date.now() });

    try {
      // Perform ARP scan
      const devices = await this.scanner.scan(job.interface, job.subnet, job.execution_time);
      ...
      const resetJobStmt = db.prepare('UPDATE jobs SET status = "active" WHERE id = ?');
      resetJobStmt.run(jobId);
  ...
  async _applyRetentionPolicy(jobId, job) {
    ...
    if (job.retention_policy === 'remove') {
      ...
      const deleteStmt = db.prepare('DELETE FROM known_devices WHERE job_id = ? AND status = "inactive"');
      ...
      const deleteStmt = db.prepare(`
        ...
        WHERE job_id = ? AND status = "inactive" 
        AND datetime(last_seen) < datetime('now', '-' || ? || ' days')
      `);
      deleteStmt.run(jobId, job.retention_days);
    }
  }
